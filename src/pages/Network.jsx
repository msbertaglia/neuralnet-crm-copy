...
export default function Network() {
  ...
  const [originalCentralId, setOriginalCentralId] = useState(null);
    ...
    <div className="h-screen w-full bg-slate-950 flex flex-col">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-800 bg-slate-900/80 backdrop-blur-sm flex-shrink-0 relative z-50">
        ...
        <div className="hidden md:flex items-center gap-2">
          <StatPill label="Total" value={`${filteredContacts.filter(c => c.introduced_by_id && c.introduced_by_id !== "sem_informacao").length} de ${stats.total}`} color="text-slate-300" />
        ...
        <div className="flex items-center gap-1.5 bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1">
          <span className="text-slate-400 text-xs whitespace-nowrap hidden sm:inline">Dist.</span>
          <select
            value={selectedOrbit}
            onChange={e => {
              const lvl = Number(e.target.value);
              setSelectedOrbit(lvl);
              setOrbitInputValue(String(orbitDistances[lvl] ?? 180));
            }}
      ...
      <div className="flex-1 relative overflow-hidden">
        {loading ? (
          ...
          <NetworkGraph
            ...
            originalCentralId={originalCentralId}
            onSetCenter={(contact) => {
